...
public class ConfigAutoOpMode extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive()) {
            if (AutoConfiguration.autonomousConfigured && (!warningHandled)) {
                if (gamepad1.x) {
                    telemetry.clear();
                    for (int x = 0; x < AutoConfiguration.commandNames.size(); x ++) {
                        telemetry.addData(AutoConfiguration.commandNames.get(x), AutoConfiguration.commandDetails.get(x));
                    }
                }
                else {
                    telemetry.addLine("Autonomous configuration detected.");
                ...
                if (gamepad1.a) {
                    warningHandled = true;
                    AutoConfiguration.commandNames.clear();
                    AutoConfiguration.commandDetails.clear();
                    AutoConfiguration.autonomousConfigured = false;
                }
                if (gamepad1.b) {
                    AutoConfiguration.commandNames.clear();
                    AutoConfiguration.commandDetails.clear();
                ...
                if (gamepad1.aWasPressed()) {
                    ...
                    AutoConfiguration.commandNames = commandNames;
                    AutoConfiguration.commandDetails = commandDetails;
                    AutoConfiguration.autonomousConfigured = true;
                }
